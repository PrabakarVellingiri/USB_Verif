...
if ("$tool" == "-cadence" ) then
  if("$cmd_option" == "-func_cov") then
    if("$multims" == "-multims") then
      irun -uvm -disable_sem2009 -TIMESCALE 1ns/1ps -v93 -messages -linedebug \
        -f ../compile/tb_multims_comp_list.fl\
  ...
  if( "$tool" == "-vcs" ) then
    ...
    -f ../compile/tb_comp_list.fl | tee -a $testcase_name.log
     if ( $vcs_seed == 1 ) then
       echo "********************************************" | tee -a $testcase_name.log 
...
if ("$tool" == "-questa" ) then
  if("$cmd_option" == "-func_cov") then
     if("$multims" == "-multims") then
      ...
      vlog -sv -permissiive -novopt +define+QUESTA_SIM +define+UVM_NO_DPI +define+QUESTA \
        +incdir+$UVM_HOME/ $UVM_HOME/uvm_pkg.sv $UVM_HOME/dpi/uvm_dpi.cc $extdef \
          -f ../compile/tb_multims_comp_list.fl | tee -a $testcase_name.log 
	...
	$tcl_opt \
     ...
     if("$multims" == "-multims") then
       ...
       +define+UVM_NO_DPI +define+QUESTA +incdir+$UVM_HOME $UVM_HOME/uvm_pkg.sv $UVM_HOME/dpi/uvm_dpi.cc $extdef \
           -f ../compile/tb_multims_comp_list.fl | tee $testcase_name.log
       ...
       vlog -sv -permissive -novopt \
       +define+UVM_NO_DPI +define+QUESTA +incdir+$UVM_HOME $UVM_HOME/uvm_pkg.sv $UVM_HOME/dpi/uvm_dpi.cc $extdef \
           -f ../compile/tb_comp_list.fl | tee $testcase_name.log
       vsim -novopt -gui $top_module_name \
           +UVM_TESTNAME=$testcase_name \
           +UVM_VERBOSITY=$verbosity  \
	   $tcl_opt \
    ...
    if("$multims" == "-multims") then
      ...
      vlib work
      vlog -sv -sva -permissive -novopt +define+UVM_NO_DPI +define+QUESTA +incdir+$UVM_HOME $UVM_HOME/uvm_pkg.sv $UVM_HOME/dpi/uvm_dpi.cc $extdef \
          -f ../compile/tb_comp_list.fl | tee $testcase_name.log
    ...
    if("$multims" == "-multims") then
      ...
      vlog -sv -permissive -novopt +define+QUESTA +define+QUESTA_SIM \
          -f ../compile/tb_multims_comp_list.fl $extdef  | tee $testcase_name.log 
      vsim -novopt -debugDB -c -permissive $top_module_name \
          +UVM_TESTNAME=$testcase_name \
          +UVM_VERBOSITY=$verbosity  \
	   $tcl_opt \
	  ...
	  -do "$dump_questa; run -a; quit -f" | tee -a $testcase_name.log
        ...
        vlib work
      vlog -sv -permissive -novopt +define+QUESTA +define+QUESTA_SIM \
          -f ../compile/tb_comp_list.fl $extdef  | tee $testcase_name.log 
      vsim -novopt -c -permissive $top_module_name \
          +UVM_TESTNAME=$testcase_name \
          +UVM_VERBOSITY=$verbosity  \
	   $tcl_opt \
